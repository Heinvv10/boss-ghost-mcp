...
describe('Self-Healing Selector System', () => {
    ...
    describe('Healing Strategies', () => {
      ...
      `);
            const result = await selector.findElement(page, '.btn-primary', { textContent: 'Submit Form' });
            expect(result.element).not.toBeNull();
            expect(result.healingApplied).toBe(true);
      ...
      `);
            const result = await selector.findElement(page, '.missing-class', { textContent: 'Click Me' });
            expect(result.element).not.toBeNull();
            expect(result.healingApplied).toBe(true);
