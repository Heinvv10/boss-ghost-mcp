...
export class AutonomousExplorer {
     ...
     */
    async explore(page, startUrl, config = {}) {
        const fullConfig = {
            maxDepth: 3,
            maxPages: 50,
            followExternal: false,
        ...
        return {
            ...
            stats: {
                ...
                totalLinks: Array.from(this.sitemap.values()).reduce((sum, p) => sum + p.links.length, 0),
     ...
     */
    async visitPage(page, url, depth, config) {
        const startTime = Date.now();
        const errors = [];
     ...
     */
    shouldIgnoreUrl(url, ignorePatterns) {
