 ...
 */
function resolveFromConfig(name, config, portIndex) {
    const cdpPort = config.cdpPort ?? CDP_PORT_RANGE_START + portIndex;
    if (cdpPort < CDP_PORT_RANGE_START || cdpPort > CDP_PORT_RANGE_END) {
        logger('Profile "%s" CDP port %d is outside range %d-%d', name, cdpPort, CDP_PORT_RANGE_START, CDP_PORT_RANGE_END);
    }
    return {
        name,
        cdpPort,
...
export function allocateCdpPort(usedPorts) {
    ...
    throw new Error(`No available CDP ports in range ${CDP_PORT_RANGE_START}-${CDP_PORT_RANGE_END}. ` +
