 ...
 */
export function matchesAllowlist(hostname, patterns) {
 ...
 */
export function isBlockedHostname(hostname, blockedList) {
    const normalized = hostname.toLowerCase();
 ...
 */
export function validateUrl(url, policy) {
    // 1. Parse URL
    let parsed;
    try {
        parsed = new URL(url);
 ...
 */
export function containsSecrets(url) {
