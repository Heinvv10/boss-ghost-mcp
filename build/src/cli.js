        ...
        coerce: (val) => {
            ...
            for (const [key, value] of Object.entries(parsed)) {
                ...
                if (!headerNamePattern.test(key)) {
                    throw new Error(`Invalid header name "${key}": must contain only RFC 7230 token characters`);
                }
                // Validate header value: must be string
                if (typeof value !== 'string') {
                    throw new Error(`Invalid header value for "${key}": must be a string, got ${typeof value}`);
                }
                // Validate header value length
                if (value.length > MAX_HEADER_VALUE_LENGTH) {
                    throw new Error(`Header value for "${key}" exceeds maximum length of ${MAX_HEADER_VALUE_LENGTH} characters`);
                }
                headers[key] = value;
            }
            return headers;
