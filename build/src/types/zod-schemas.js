 ...
 */
export function isZodType(value) {
    return (value !== null &&
        typeof value === 'object' &&
        '_def' in value &&
 ...
 */
export function getZodSchemaShape(schema) {
    if (!isZodType(schema)) {
        return null;
    }
 ...
 */
export function getZodTypeDef(type) {
    if (!isZodType(type)) {
        return null;
    }
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
 ...
 */
export function buildZodObjectSchema(fields) {
    const shape = {};
 ...
 */
export function parseFieldType(typeString) {
    ...
    const type = (knownTypes.includes(baseType) ? baseType : 'unknown');
    return {
        type,
        optional: isOptional,
    };
...
export function describeZodSchema(schema) {
    ...
    for (const [fieldName, fieldSchema] of Object.entries(shape)) {
        const def = getZodTypeDef(fieldSchema);
        if (!def) {
        ...
        if (typeString === 'optional') {
            const innerDef = 
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            def.innerType ? getZodTypeDef(def.innerType) : null;
        ...
        // Handle array types
            ...
            const elementDef = 
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            def.element ? getZodTypeDef(def.element) : null;
