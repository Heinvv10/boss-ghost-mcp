 ...
 */
function serializeAXTree(node, depth = 0) {
...
export const browserVision = defineTool({
    name: 'browser_vision',
    description: 'Analyze the current page visually using AI. Takes a screenshot and sends it to the calling LLM for multimodal analysis. Useful for understanding page layout, finding UI elements, or answering visual questions about the page.',
    annotations: {
    ...
    schema: {
        question: zod
            .string()
            .describe('What to analyze about the page (e.g. "What products are shown?", "Is there a login form?", "Describe the layout")'),
        includeSnapshot: zod
            .boolean()
            .optional()
            .describe('Whether to also include the accessibility tree for richer analysis. Defaults to false.'),
    },
    handler: async (request, response, context) => {
        ...
        if (analysisContext) {
            response.appendResponseLine(wrapExternalContent(analysisContext, pageUrl));
        }
...
export const summarizePage = defineTool({
    name: 'summarize_page',
    description: 'Get a task-focused summary of the current page content. Takes an accessibility snapshot and returns a concise summary focused on the specified task or question. Useful when page snapshots are too long to process.',
    annotations: {
        category: ToolCategory.DEBUGGING,
        readOnlyHint: true,
    },
    schema: {
        task: zod
            .string()
            .describe('What you need from this page (e.g. "find the pricing table", "extract all product names", "locate the search functionality")'),
        maxChars: zod
            .number()
            .optional()
    ...
    handler: async (request, response, context) => {
        ...
        response.appendResponseLine(`## Page Summary (${pageUrl})`);
        response.appendResponseLine(`Full page is ${fullSnapshot.length} characters. Showing task-focused extract for: "${request.params.task}"`);
        response.appendResponseLine('');
        // Split into lines and score relevance by keyword overlap
        const taskWords = new Set(request.params.task
            .toLowerCase()
            .split(/\s+/)
            .filter(w => w.length > 2));
        ...
        for (const item of scoredLines) {
            if (item.score === 0 && charCount > maxChars * 0.5)
                break;
            if (charCount + item.line.length > maxChars)
                break;
        ...
        response.appendResponseLine(wrapExternalContent(relevantLines.join('\n'), pageUrl));
