...
export const savePageState = defineTool({
    ...
    handler: async (request, response, context) => {
        ...
        await memory.savePageState(request.params.sessionId, page, request.params.includeFormData);
...
export const restorePageState = defineTool({
    ...
    handler: async (request, response, context) => {
        ...
        await memory.restorePageState(request.params.sessionId, page, request.params.restoreFormData);
    ...
    handler: async (request, response, context) => {
        ...
        try {
            ...
            const captchaDetection = {
                type: request.params.captchaType,
                confidence: 1.0,
                element: null,
                location: null,
            };
            const solved = await detector.waitForSolution(page, captchaDetection, request.params.timeout);
            if (solved) {
