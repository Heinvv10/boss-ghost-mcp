...
describe('Fingerprint Randomization', () => {
  let bossGhost1: Browser;
  let bossGhost2: Browser;
  let chrome1: Browser;
  let chrome2: Browser;
  ...
  it('BOSS Ghost MCP: canvas fingerprints should differ between instances', { timeout: 15000 }, async () => {
    ...
    const result1 = await page1.evaluate(() => {
      ...
      if (!ctx) return { dataURL: '', debugInfo: 'no context' };
      ctx.textBaseline = 'top';
      ctx.font = '14px Arial';
      ctx.fillText('Test', 2, 2);

      // Get debug info from window if available
      const debugInfo = (window as any).__GHOST_DEBUG__ || 'no debug info';
