...
export interface CaptchaDetection {
  ...
  metadata?: {
    sitekey?: string; // reCAPTCHA/hCaptcha site key
...
export class CaptchaDetector {
  ...
  async detect(page: Page): Promise<CaptchaDetection[]> {
    const captchas: CaptchaDetection[] = [];
   ...
   * Detect Cloudflare Turnstile
   */
  private async detectCloudflareTurnstile(
    page: Page,
  ): Promise<CaptchaDetection | null> {
    const result = await page.evaluate(() => {
      ...
      if (
        challengeTitle?.textContent?.includes('Just a moment') ||
   ...
   */
  private async detectImageCaptcha(page: Page): Promise<CaptchaDetection[]> {
    const results = await page.evaluate(() => {
      ...
      const hasCaptchaKeyword = keywords.some(keyword =>
  ...
  async waitForCaptcha(
    ...
    while (Date.now() - startTime < timeout) {
      ...
      if (captchas.length > 0) {
        console.log(
          `[CAPTCHA] Detected ${captchas[0].type} after ${Date.now() - startTime}ms`,
        );
        return captchas[0];
      }
  ...
  async waitForSolution(
    ...
    while (Date.now() - startTime < timeout) {
      ...
      if (!stillPresent) {
        console.log(
          `[CAPTCHA] ${captcha.type} solved after ${Date.now() - startTime}ms`,
        );
        return true;
      }
   ...
   */
  async getStats(page: Page): Promise<{
    total: number;
    byType: Record<CaptchaType, number>;
  }> {
    const captchas = await this.detect(page);

    const byType: Record<CaptchaType, number> = {
      recaptcha_v2: 0,
      recaptcha_v3: 0,
      hcaptcha: 0,
      cloudflare: 0,
