...
export class DomExtractor {
  ...
  async extract(
    ...
    const data = await page.evaluate(
      (scopeSelector, fieldTypes: Record<string, { type: string; elementType?: string }>) => {
          ...
          for (const sel of selectors) {
            ...
            if (fieldType === 'array') {
              ...
              if (fieldName === 'images' || fieldName === 'image') {
                ...
                const imageSrcs = Array.from(imgs || []).map(img => (img as HTMLImageElement).src).filter(Boolean);
