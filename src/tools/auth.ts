 ...
 */
function findAuthConfig(url: string): {site: string; config: SiteAuthConfig} | null {
...
export const browserLogin = defineTool({
  name: 'browser_login',
  description:
    'Authenticate with a website using credentials stored in ~/.boss-ghost/config.json. ' +
  ...
  schema: {
    site: zod
      ...
      .optional()
      .describe(
        'Hostname to authenticate with (e.g. "app.isaflow.co.za"). If omitted, uses the current page URL.',
      ),
  },
  handler: async (request, response, context) => {
    ...
    if (!match) {
      response.appendResponseLine(
        `No auth config found for "${targetSite ?? currentUrl}". ` +
    ...
    for (const [fieldKey, fieldValue] of Object.entries(config.fields)) {
      ...
      if (!filled) {
        ...
        try {
          const inputSelector = await page.evaluate((key: string) => {
...
export const listAuthSites = defineTool({
  ...
  handler: async (_request, response) => {
    ...
    for (const [site, config] of Object.entries(auth)) {
      ...
      response.appendResponseLine(
        `- **${site}** [${config.method}] fields: ${fields}${config.loginUrl ? ` login: ${config.loginUrl}` : ''}`,
      );
    }
  },
});
