...
function captureScreenshot(windowName?: string): string {
  const tmpPath = join('/tmp', `bg-desktop-${Date.now()}.png`);

  if (windowName) {
    try {
      const winId = execSync(
        `xdotool search --name "${windowName.replace(/"/g, '\\"')}" | head -1`,
        {encoding: 'utf8'},
      ).trim();
      if (!winId) throw new Error(`Window "${windowName}" not found`);
...
function screenshotToBase64(imagePath: string): {
  dataUrl: string;
  width: number;
  height: number;
  base64: string;
...
async function callVlm(
  ...
  if (!vlmConfigured()) {
    throw new Error(
      'Desktop VLM tools require VLM_API_URL and VLM_MODEL environment variables',
    );
  }
...
function parseJsonFromVlm(raw: string): unknown {
  const match = raw.match(/\[[\s\S]*\]|\{[\s\S]*\}/);
  if (!match) throw new Error('VLM did not return valid JSON');
...
export const desktopScreenshot = defineTool({
  ...
  schema: {
    windowName: zod
      ...
      .optional()
      .describe(
        'Title of a specific window to capture. If omitted, captures the full desktop.',
      ),
    describe: zod
      .boolean()
      .default(true)
      .describe(
        'If true, sends the screenshot to the VLM for a description. Set to false to just get the raw image.',
      ),
  },
  handler: async (request, response) => {
    ...
    response.appendResponseLine(
      `Desktop screenshot captured (${width}x${height})`,
    );

    if (request.params.describe && vlmConfigured()) {
      const description = await callVlm(
        dataUrl,
        `Describe what you see on this desktop screenshot concisely. Include: visible applications, what the user is working on, and key UI elements with their approximate locations.`,
        1024,
      );
      response.appendResponseLine('');
...
export const desktopAction = defineTool({
  ...
  schema: {
    instruction: zod
      .string()
      .describe(
        'Natural language description of what to click, e.g. "the Save button", "Firefox icon in the dock", "File menu".',
      ),
    action: zod
    ...
    windowName: zod
      ...
      .optional()
      .describe(
        'Optional: capture only this window instead of the full desktop.',
      ),
    dryRun: zod
      .boolean()
      .default(false)
      .describe(
        'If true, only identifies the element and returns coordinates without actually clicking.',
      ),
  },
  handler: async (request, response) => {
    ...
    const vlmResult = await callVlm(
      dataUrl,
      `You are a desktop automation assistant. Look at this screenshot (${width}x${height} pixels) and find the UI element described below.
...
Return ONLY the JSON object, no other text.`,
    ...
    try {
      parsed = parseJsonFromVlm(vlmResult) as typeof parsed;
    } catch {
    ...
    response.attachImage({mimeType: 'image/jpeg', data: base64});
    response.appendResponseLine(
      `Found: "${parsed.element}" at (${x}, ${y}) — confidence: ${parsed.confidence}`,
    );
    ...
    if (parsed.confidence < 0.5) {
      response.appendResponseLine(
        `Confidence too low (${parsed.confidence}) — skipping click. Use dryRun:false to force.`,
      );
      return;
    }
...
export const desktopType = defineTool({
  ...
  schema: {
    ...
    clickTarget: zod
      ...
      .optional()
      .describe(
        'Optional: natural language description of where to click first before typing (e.g. "the search box", "the URL bar").',
      ),
    pressEnter: zod
      .boolean()
      .default(false)
  ...
  handler: async (request, response) => {
    ...
    if (clickTarget) {
      ...
      const vlmResult = await callVlm(
        dataUrl,
        `Find the UI element: "${clickTarget}". Return JSON: {"coordinates": [x, y], "confidence": 0-1}. ONLY JSON.`,
        256,
      );

      try {
        const parsed = parseJsonFromVlm(vlmResult) as {
          coordinates: [number, number];
          confidence: number;
        };
        const [x, y] = parsed.coordinates;
        ...
        response.appendResponseLine(
          `Could not locate "${clickTarget}" — typing at current focus`,
        );
      }
    }
    ...
    response.appendResponseLine(
      `Typed "${text.length > 50 ? text.slice(0, 50) + '...' : text}"${pressEnter ? ' + Enter' : ''}`,
    );
  },
});
...
export const desktopKey = defineTool({
  ...
  schema: {
    key: zod
      .string()
      .describe(
        'Key or key combination to press. Use xdotool format: "ctrl+s", "alt+F4", "Return", "Tab", "ctrl+shift+t", "super".',
      ),
  },
  handler: async (request, response) => {
...
export const desktopFindElements = defineTool({
  ...
  schema: {
    ...
    elementType: zod
      ...
      .optional()
      .describe(
        'Optional: filter by type, e.g. "button", "text_field", "menu_item", "tab".',
      ),
  },
  handler: async (request, response) => {
    ...
    const vlmResult = await callVlm(
      dataUrl,
      `Analyze this desktop screenshot (${width}x${height}) and identify the main interactable UI elements. ${typeFilter}
...
Return ONLY the JSON array.`,
    ...
    try {
      const elements = parseJsonFromVlm(vlmResult) as Array<{
        type: string;
        label: string;
      ...
      response.appendResponseLine(
        `Found ${elements.length} UI elements (${width}x${height}):`,
      );
      response.appendResponseLine('');
      for (const el of elements) {
        response.appendResponseLine(
          `  [${el.type}] "${el.label}" at (${el.center[0]}, ${el.center[1]})`,
        );
      }
    } catch {
  ...
  handler: async (_request, response) => {
    try {
      const output = execSync(
        `wmctrl -l 2>/dev/null || xdotool search --name "" getwindowname %@ 2>/dev/null | head -50`,
      ...
      try {
        const output = execSync(
          `xdotool search --onlyvisible --name "" 2>/dev/null | while read wid; do echo "$wid: $(xdotool getwindowname $wid 2>/dev/null)"; done | head -30`,
