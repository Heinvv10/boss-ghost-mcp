...
- optional: Add "?" suffix (e.g., "string?" for optional string)`,
  ...
  schema: {
    schema: zod
      ...
      .describe(
        'JSON Schema representation where keys are field names and values are types (string, number, boolean, string[], number[], etc.)'
      ),
    extractionMode: zod
      .enum(['dom', 'llm', 'hybrid'])
      .default('dom')
      .describe(
        'Extraction method: "dom" for fast DOM parsing, "llm" for AI-powered extraction, "hybrid" for DOM with LLM fallback'
      ),
    selector: zod
      .string()
      .optional()
      .describe(
        'Optional CSS selector to limit extraction scope to a specific element'
      ),
    llmInstructions: zod
      .string()
      .optional()
      .describe(
        'Additional natural language instructions for LLM extraction (only used in llm or hybrid mode)'
      ),
  },
      ...
      // Mode 3: Hybrid (DOM first, LLM fallback)
        ...
        try {
          ...
          response.appendResponseLine(
            `⚠️ DOM extraction failed: ${domError instanceof Error ? domError.message : String(domError)}`
          );
 ...
 */
function buildZodSchema(schemaShape: Record<string, string>): zod.ZodObject<any> {
